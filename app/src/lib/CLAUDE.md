...
## Key Files
- `ddb.ts` — DynamoDB read path (hot store; bounded-concurrency pool `K=40` for the per-bucket flow-query fan-out); `cw-metrics.ts` — CloudWatch metrics + `healthByMonitor`; `cw-alarms.ts` — CloudWatch alarm states; `ssm.ts` — cached SSM params (e.g. `/nfm-dashboard/gateway-url`)
