...
- `lib/app-stack.ts` — ECR image → ECS Fargate (arm64) behind ALB, fronted by CloudFront, Cognito Hosted UI (PKCE). Header comment documents the ALB → CloudFront → Cognito circular-dependency resolution — read it before touching ordering. The `authDisabled` context injects `AUTH_DISABLED=1` into the task env, disabling only the middleware session gate (Cognito resources + origin-verify stay; ADR-005). Currently OFF — login enforced.
