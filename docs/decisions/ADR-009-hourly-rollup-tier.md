...
1. **Key/shape**: `pk: HFLOW#<hourBucket>#<monitor>`, `sk: <metric>#<category>#<edgeHash>` — the hour-bucket formula mirrors the collector's 5-min formula at a 3,600,000 ms grain. No GSI attributes (the pod/edge time-series indexes stay 5-min-only; GSI keys carried by raw read-backs are stripped explicitly). TTL 15 days — the 7d pair path reads up to ~2×168+1 closed hours, so hourly rows must outlive ~2 weeks; the `HROLL#done` markers share the TTL.
...
3. **Hour-close job**: appended to the existing 5-minute collector cycle, isolated by try/catch so a rollup failure never fails the collect cycle. Eligibility = hour boundaries at least one cycle past close (their last 5-min bucket has landed). A `HROLL#done` marker in the meta table drives idempotent, resumable processing — up to 6 missing hours per cycle, newest first — so a fresh deploy auto-backfills the last 7 days in roughly 28 cycles (~2.3 h). The marker is **withheld when BatchWrite drops items** after its retries: a dropped item means a silently incomplete hour-sum, so the hour stays unmarked and is retried next cycle instead of undercounting permanently.
4. **App read side**: `windowPlan`/`windowPairPlan` (`app/src/lib/ddb.ts`) route requests over 36 buckets (>3h) to closed-hour HFLOW rows plus a live 5-minute tail; ≤36 buckets stay on the unchanged raw path. The **newest closed hour is always read at raw 5-min grain** — its HFLOW row lands only ~5–15 minutes after the hour closes, so reading it hourly would drop that hour from every >3h view once per hour (no hourly sawtooth). The pair (window-over-window) path is symmetric — closed hours only, no tail on either half, both halves shifted one hour back so neither includes the possibly-not-yet-rolled newest closed hour — so movers/anomalies deltas aren't biased toward the current window. The pair routes additionally guard on data-derived prior coverage: below 80% expected prior-hour buckets they return an honest empty result (`insufficientPrior: true`) instead of fake spikes.
...
- Storage cost is negligible: rollup rows are capped at 200 per (hour, monitor, metric, category) and TTL out after 15 days — an order of magnitude smaller footprint than the raw 5-min feed they summarize. The archive pipeline is untouched, so it stays raw-only.
...
- Until hourly retention covers the prior window (~7 days after the first deploy, or after any retention hole), the 7d movers/anomalies pair lenses serve an empty result with `insufficientPrior: true` rather than reporting a false spike on every entity from a half-empty prior half.
...
1. **키/형태**: `pk: HFLOW#<hourBucket>#<monitor>`, `sk: <metric>#<category>#<edgeHash>` — 시간 버킷 공식은 수집기의 5분 공식을 3,600,000ms 그레인으로 그대로 반영합니다. GSI 속성 없음(pod/edge 시계열 인덱스는 5분 전용으로 유지; 원시 read-back이 실어 오는 GSI 키는 명시적으로 제거). TTL 15일 — 7d pair 경로가 최대 ~2×168+1개의 마감된 시간을 읽으므로 시간 단위 row는 약 2주 이상 유지되어야 하며, `HROLL#done` 마커도 같은 TTL을 공유합니다.
...
3. **시간 마감(hour-close) 작업**: 기존 5분 수집기 사이클에 덧붙이며, try/catch로 격리되어 rollup 실패가 수집 사이클 자체를 실패시키지 않습니다. 대상 시간 = 마감 후 최소 한 사이클이 지난 시간 경계(해당 시간의 마지막 5분 버킷이 이미 적재됨). meta 테이블의 `HROLL#done` 마커가 idempotent하고 재개 가능한 처리를 주도합니다 — 사이클당 최대 6개 누락 시간을 최신순으로 처리하여, 신규 배포 시 최근 7일이 약 28사이클(~2.3시간)에 자동 백필됩니다. BatchWrite가 재시도 후에도 아이템을 드롭하면 **마커를 기록하지 않습니다**: 드롭된 아이템은 조용히 불완전한 시간 합계를 의미하므로, 해당 시간은 미기록 상태로 남아 다음 사이클에 재시도되어 영구적인 과소 집계를 방지합니다.
4. **앱 읽기 측**: `windowPlan`/`windowPairPlan`(`app/src/lib/ddb.ts`)이 36버킷(3시간) 초과 요청을 마감된 시간 HFLOW row + 실시간 5분 tail로 라우팅하고, 36버킷 이하는 기존 원시 경로를 그대로 유지합니다. **가장 최근에 마감된 시간은 항상 원시 5분 그레인으로 읽습니다** — 해당 HFLOW row는 시간 마감 후 ~5–15분 뒤에야 적재되므로, 시간 단위로 읽으면 매시간 한 번씩 모든 >3h 뷰에서 그 시간이 빠지게 됩니다(hourly sawtooth 없음). pair(윈도우 대비) 경로는 대칭적입니다 — 마감된 시간만 사용, 양쪽 절반 모두 tail 없음, 양쪽 절반을 1시간 뒤로 이동해 아직 rollup되지 않았을 수 있는 최신 마감 시간을 아예 제외 — 그래서 movers/anomalies 델타가 현재 윈도우 쪽으로 편향되지 않습니다. pair 라우트는 추가로 데이터 기반 prior 커버리지를 검사합니다: 기대되는 prior 시간 버킷의 80% 미만이면 가짜 급증 대신 정직한 빈 결과(`insufficientPrior: true`)를 반환합니다.
...
- 저장 비용은 무시할 수준: rollup row는 (시간, 모니터, 메트릭, 카테고리)당 200개로 상한되고 15일 후 TTL로 소멸하여, 이를 요약하는 원시 5분 피드보다 자릿수 단위로 작은 용량입니다. 아카이브 파이프라인은 영향받지 않으므로 raw-only로 유지됩니다.
...
- 시간 단위 보존(retention)이 prior 윈도우를 덮을 때까지(최초 배포 후 약 7일, 또는 보존 구멍 이후), 7d movers/anomalies pair lens는 반쯤 빈 prior 절반으로 모든 엔티티에 가짜 급증을 보고하는 대신 `insufficientPrior: true`와 함께 빈 결과를 제공합니다.
