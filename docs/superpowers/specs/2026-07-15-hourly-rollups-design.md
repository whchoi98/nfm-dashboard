...
- **TTL: 15 days** (the pair path reads up to ~2×168+1 closed hours, so hourly rows must outlive ~2 weeks).

Completion marker (meta table): `pk: 'HROLL#done', sk: <hourBucket>`, `ttl` shares the 15d rollup TTL (the 168h re-roll lookback stays far below it). One marker per hour (not per monitor — an hour is rolled up for all monitors atomically in one pass). The marker is withheld when BatchWrite drops items after retries — a dropped item means a silently incomplete hour-sum, so the idempotent hour is retried next cycle instead.
...
- **Hourly path window:** quantize the request to `H = round(n/12)`. The **newest closed hour is ALWAYS read at raw 5-min grain** — its HFLOW row lands only ~5–15 min after the hour closes (collector rollup lag), so reading it hourly would drop that hour from every >3h view once per hour (the sawtooth). The plan is therefore `H−1` hourly parts (from the SECOND newest closed hour backwards) + a raw tail of the open hour's 5-min buckets plus the newest closed hour's 12; total window seconds unchanged. Pair fetch: one clock read, `2H` **closed hours only**, shifted one hour back (both halves skip the newest closed hour entirely — it may not be rolled up yet), split symmetrically H/H — no tail on either half, because an asymmetric tail would bias every window-over-window delta (movers/anomalies) toward the current window; the pair path trades ≤2 h of freshness for symmetry. The 7d pair lenses (movers/anomalies) additionally guard on data-derived prior coverage: when the prior half holds <80% of its expected hour buckets (e.g. while hourly retention accumulates in the first ~7 days after deploy), the routes return an honest empty result (`insufficientPrior: true`) instead of feeding a half-empty prior into the lens and reporting fake spikes on every entity.
