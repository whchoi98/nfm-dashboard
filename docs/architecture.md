...
- **저장(Storage, 핫)** — DynamoDB 단일 테이블 설계: `nfm-dashboard-flows`(플로우 레코드, 7일 TTL, pod/edge 시계열 GSI, DynamoDB Stream `NEW_IMAGE`) + 동일 테이블의 두 번째 시간 단위 rollup 계층(`HFLOW#<hour>#<monitor>` row, GSI 속성 없음, top-200 상한, 8일 TTL, 수집기 hour-close 단계가 기록 — ADR-009)와 `nfm-dashboard-meta`(토폴로지 스냅샷, 수집 상태/이력, 커버리지, `WI#latest`, `DNS#latest`, `HROLL#done` rollup 마커). 키 기반 접근만 사용.
