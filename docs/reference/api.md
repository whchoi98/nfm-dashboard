...
- Interactive lens ranges cap at 24h: `parseLensParams` clamps `?buckets` to [1, 288]; 7d+ analysis is served by the Athena-backed `/api/history` (ADR-008).
...
- 인터랙티브 lens 범위는 24h 상한: `parseLensParams`가 `?buckets`를 [1, 288]로 클램프; 7d+ 분석은 Athena 기반 `/api/history`가 담당합니다(ADR-008).
