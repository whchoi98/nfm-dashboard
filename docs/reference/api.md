...
### 1. Overview
The API layer is a set of Next.js App Router route handlers under `app/src/app/api/` — 22 `route.ts` files serving dashboard data, analytics, auth, health, the SSE AI chat, and the Athena-backed flow archive query. All routes except `/api/health` and `/api/auth/*` are gated by `app/src/middleware.ts`.
...
| Analytics | `app/src/app/api/analytics/{latency,reliability,cost,dns,dependencies,efficiency,scorecard,movers}/route.ts` | Aggregations via `app/src/lib/analytics/*` |
...
| New menus (Phase 8) | `app/src/app/api/{alerts,search,anomalies,cost-explorer,reports}/route.ts` | Alerts (CloudWatch alarms + derived event feed), unified entity search, baseline anomaly detection, cost explorer, report data |
...
### 1. 개요
API 계층은 `app/src/app/api/` 아래 Next.js App Router route handler 모음이다 — 22개의 `route.ts`가 대시보드 데이터, 분석, 인증, 헬스체크, SSE AI 채팅, Athena 기반 flow 아카이브 조회를 제공한다. `/api/health`와 `/api/auth/*`를 제외한 모든 라우트는 `app/src/middleware.ts`가 보호한다.
...
| 분석 | `app/src/app/api/analytics/{latency,reliability,cost,dns,dependencies,efficiency,scorecard,movers}/route.ts` | `app/src/lib/analytics/*` 기반 집계 |
...
| 신규 메뉴 (Phase 8) | `app/src/app/api/{alerts,search,anomalies,cost-explorer,reports}/route.ts` | 알림(CloudWatch 알람 + 파생 이벤트 피드), 통합 엔티티 검색, baseline 이상 탐지, 비용 탐색, 리포트 데이터 |
