...
| New menus (Phase 8) | `app/src/app/api/{alerts,search,anomalies,cost-explorer,reports,network}/route.ts` | Alerts (CloudWatch alarms + derived event feed), unified entity search, baseline anomaly detection, cost explorer, report data, network |
...
| 신규 메뉴 (Phase 8) | `app/src/app/api/{alerts,search,anomalies,cost-explorer,reports,network}/route.ts` | 알림(CloudWatch 알람 + 파생 이벤트 피드), 통합 엔티티 검색, baseline 이상 탐지, 비용 탐색, 리포트 데이터, network |
