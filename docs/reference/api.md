...
### 1. Overview
The API layer is a set of Next.js App Router route handlers under `app/src/app/api/` — 21 `route.ts` files serving dashboard data, analytics, auth, health, and the SSE AI chat. All routes except `/api/health` and `/api/auth/*` are gated by `app/src/middleware.ts`.
...
### 1. 개요
API 계층은 `app/src/app/api/` 아래 Next.js App Router route handler 모음이다 — 21개의 `route.ts`가 대시보드 데이터, 분석, 인증, 헬스체크, SSE AI 채팅을 제공한다. `/api/health`와 `/api/auth/*`를 제외한 모든 라우트는 `app/src/middleware.ts`가 보호한다.
