#!/bin/bash
# Scaffolding test runner for nfm-dashboard (harness engineering tests).
# Distinct from the app/collector vitest suites — validates hooks, secret
# patterns, and repo structure.
#
...
assert_json_valid() {
    ...
    if command -v node >/dev/null 2>&1; then
        node -e "JSON.parse(require('fs').readFileSync('$filepath','utf8'))" > /dev/null 2>&1 \
            && pass "$desc" || fail "$desc" "invalid JSON: $filepath"
    else
...
export -f assert_dir_exists assert_file_executable assert_json_valid assert_bash_syntax
...
echo -e "${CYAN}=== nfm-dashboard Scaffolding Test Suite ===${NC}"
echo ""
