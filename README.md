...
- **24h interactive analytics, cached to the collector cycle** — Interactive lens ranges cover 15m–24h (server-side clamp, `MAX_BUCKETS` 288) and serve from a version-aligned in-process cache that stays valid until the collector writes a new cycle (ADR-007); 7d+ analysis runs on the Athena-backed History page over the S3 Parquet archive (ADR-008).
...
- **수집 주기에 정렬된 캐시 기반 24h 인터랙티브 분석** — 인터랙티브 lens 범위는 15m–24h(서버측 클램프, `MAX_BUCKETS` 288)이며, 수집기가 새 사이클을 기록할 때까지 유효한 버전 정렬 인프로세스 캐시에서 응답합니다(ADR-007); 7d+ 분석은 S3 Parquet 아카이브 위의 Athena 기반 History 페이지가 담당합니다(ADR-008).
