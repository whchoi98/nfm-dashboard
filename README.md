...
- **15m–7d interactive analytics on hourly rollups, cached to the collector cycle** — Interactive lens ranges cover 15m–7d (server-side clamp, `MAX_BUCKETS` 2016): requests over 3h read closed-hour rollup rows the collector writes each hour (`HFLOW#<hour>#<monitor>`) plus a live 5-minute tail instead of a raw fan-out (ADR-009), and responses serve from a version-aligned in-process cache that stays valid until the collector writes a new cycle (ADR-007); beyond 7 days, the Athena-backed History page queries the S3 Parquet archive.
...
- **Cost notes** (rough): always-on cost is 1 ECS Fargate task (1 vCPU/4GB, arm64) + ALB + NAT GW traffic. Usage-based cost is DynamoDB on-demand, Collector/tool Lambdas (5-minute cycle), CloudFront, NFM queries, and Bedrock tokens when chat/diagnosis is used. Bedrock cost is 0 when unused.
...
- **시간별 rollup 기반 15m–7d 인터랙티브 분석, 수집 주기에 정렬된 캐시** — 인터랙티브 lens 범위는 15m–7d(서버측 클램프, `MAX_BUCKETS` 2016)이며, 3시간 초과 조회는 원시 fan-out 대신 수집기가 매 시간 기록하는 마감된 시간 rollup row(`HFLOW#<hour>#<monitor>`) + 실시간 5분 tail을 읽습니다(ADR-009), 응답은 수집기가 새 사이클을 기록할 때까지 유효한 버전 정렬 인프로세스 캐시에서 제공합니다(ADR-007); 7일을 넘는 분석은 S3 Parquet 아카이브 위의 Athena 기반 History 페이지가 담당합니다.
...
- **비용 참고**(대략): 상시 비용은 ECS Fargate 태스크 1개(1 vCPU/4GB, arm64) + ALB + NAT GW 트래픽. 사용량 기반 비용은 DynamoDB 온디맨드, Collector/도구 Lambda(5분 주기), CloudFront, NFM 쿼리, 그리고 챗/진단 사용 시 Bedrock 토큰. 미사용 시 Bedrock 비용은 0입니다.
